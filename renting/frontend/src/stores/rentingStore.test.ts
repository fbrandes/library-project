...
describe('rentingStore helpers', () => {
  ...
  it('returns current rented orders and books', () => {
    ...
    expect(getCurrentRentedOrders([active, returned, otherUser], 'user-123')).toEqual([active]);
    expect(getCurrentRentedBooks([active, returned], 'user-123')).toEqual(active.contents);
  });
...
describe('createRentingStore', () => {
  ...
  it('loads one order and handles null', async () => {
    const store = createRentingStore(api);
    api.getOrder.mockResolvedValueOnce(orderFixture()).mockResolvedValueOnce(null);
  ...
  it('keeps existing data when mutations fail', async () => {
    ...
    await store.getState().rentBooks({ userId: 'user-123', contents: orderFixture().contents });
    expect(store.getState().error).toBe('Create failed');

    await store.getState().updateOrder(orderFixture().id, { userId: 'user-123', contents: orderFixture().contents });
  ...
  it('returns current rented books from store state', () => {
    ...
    expect(store.getState().getCurrentRentedBooks()).toEqual(orderFixture().contents);
  });
});
