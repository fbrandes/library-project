...
import { ApiError, createOrder, deleteOrder, getOrder, getOrders, updateOrder } from './rentingApi';
...
describe('rentingApi', () => {
  ...
  it('creates and updates orders with JSON payloads', async () => {
    const input = { userId: 'user-123', contents: orderFixture().contents, state: 'PLACED' as const };
    fetchMock.mockResolvedValueOnce(jsonResponse(orderFixture()));
    fetchMock.mockResolvedValueOnce(jsonResponse(orderFixture({ state: 'PROCESSED' })));
  ...
  it('deletes orders', async () => {
    fetchMock.mockResolvedValue(new Response('', { status: 204, statusText: 'No Content' }));
  ...
  it('throws ApiError for failed responses', async () => {
    ...
    await expect(getOrders()).rejects.toMatchObject(new ApiError(500, 'Broken'));
  });
});
