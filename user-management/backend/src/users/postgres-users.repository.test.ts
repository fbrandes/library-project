...
import { createdAt, updatedAt, user, userId, userInput } from './test-fixtures.js';
import { mapUserRow, PostgresUsersRepository, toIsoString } from './postgres-users.repository.js';
...
describe('PostgresUsersRepository', () => {
  it('maps rows and string dates', () => {
    ...
    expect(mapUserRow(row({ created_at: createdAt, updated_at: updatedAt }))).toEqual(user());
  });
  ...
  it('updates users and returns null when no row is updated', async () => {
    const { pool, query } = poolStub([{ rows: [row({ status: 'INACTIVE' })] }, { rows: [] }]);
    const repository = new PostgresUsersRepository(pool);

    await expect(repository.update(userId, userInput({ status: 'INACTIVE' }), updatedAt)).resolves.toMatchObject({
      status: 'INACTIVE',
    });
    await expect(repository.update(userId, userInput(), updatedAt)).resolves.toBeNull();
