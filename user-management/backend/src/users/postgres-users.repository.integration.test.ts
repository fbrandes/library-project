...
describe.skipIf(process.env.USER_MANAGEMENT_TESTCONTAINERS !== 'true')('PostgresUsersRepository integration', () => {
  it('persists a user lifecycle with Testcontainers', async () => {
    ...
    try {
      ...
      const updated = await repository.update(userId, userInput({ status: 'INACTIVE' }), new Date().toISOString());
