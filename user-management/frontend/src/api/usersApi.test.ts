...
function response(body: unknown, init: ResponseInit = {}) {
  return new Response(init.status === 204 ? null : body === undefined ? '' : JSON.stringify(body), {
...
describe('usersApi', () => {
  ...
  it('gets, updates, and deletes users', async () => {
    const fetcher = vi
      ...
      .mockResolvedValueOnce(response(undefined, { status: 204 })) as unknown as Fetcher;
    const api = createUsersApi('/api', fetcher);

    await expect(api.getUser(userId)).resolves.toEqual(user());
    await expect(api.updateUser(userId, userInput({ status: 'INACTIVE' }))).resolves.toMatchObject({
      status: 'INACTIVE',
    });
    await expect(api.deleteUser(userId)).resolves.toBeUndefined();

    expect(fetcher).toHaveBeenNthCalledWith(1, `/api/users/${encodeURIComponent(userId)}`, {
      method: 'GET',
      headers: { Accept: 'application/json' },
    });
    expect(fetcher).toHaveBeenNthCalledWith(3, `/api/users/${encodeURIComponent(userId)}`, {
      method: 'DELETE',
      headers: { Accept: 'application/json' },
    });
  });
  ...
  it('throws API errors with response details', async () => {
    const fetcher = vi.fn(async () =>
      response({ message: 'Invalid user payload', problems: ['email is required'] }, { status: 400 }),
  ...
  it('throws fallback API errors for empty error responses', async () => {
    const fetcher = vi.fn(async () => response(undefined, { status: 500 })) as unknown as Fetcher;
    const api = createUsersApi('/api', fetcher);

    await expect(api.getUser(userId)).rejects.toBeInstanceOf(ApiError);
    await expect(api.getUser(userId)).rejects.toThrow('Request failed with status 500');
  });
});
