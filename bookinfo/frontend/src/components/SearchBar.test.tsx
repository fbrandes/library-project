...
function SearchBarHarness({ isLoading = false, onSubmit = vi.fn() }: SearchBarHarnessProps) {
...
describe('SearchBar', () => {
  it('updates the selected field and query before submit', async () => {
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: 'Search by' }), 'author');
    await user.type(screen.getByLabelText('Search books'), 'Joshua Bloch');
    await user.click(screen.getByRole('button', { name: 'Search' }));

    expect(screen.getByRole('combobox', { name: 'Search by' })).toHaveValue('author');
