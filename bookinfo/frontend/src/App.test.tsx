...
describe("App", () => {
  ...
  it("loads books and filters displayed results", async () => {
    ...
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Filter by" }),
      "author",
    );
    await user.type(screen.getByLabelText("Filter results"), "Bloch");

    expect(
      screen.getByRole("heading", { name: "Effective Java" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Domain-Driven Design" }),
  ...
  it("searches by title and shows matching books", async () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "Effective Java" }),
    ).not.toBeInTheDocument();
  });
});
